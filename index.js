  ...
  connection.query(q, values, (err, result) => {
    ...
    if (result.length === 1) {
      ...
      if (password === data.password) {
        return res.render("employee.ejs", { data, loginTime });
      }
    }
...
app.post("/student/attendance", (req,res)=>{
  ...
  const query =
  "INSERT INTO student_attendence (student_id, attendance_date, status , name) VALUES (?, ?, ?,?)";
connection.query(
  query,
    ...
    if (err) {
      ...
      connection.query("SELECT LAST_INSERT_ID() as request_id", (err, rows) => {
        if (err) {
          ...
          res.send(`Leave Application submitted check your status with request_id: ${request_id}`);
        }
      });
    }
  });
...
app.post("/leave/status", (req,res)=>{
  ...
  const attendance_date = dateObject.toISOString().split("T")[0];
    ...
    connection.query(selectQuery, [employee_id, attendance_date], (err, results) => {
      if (err) {
  ...
  // When done, don't forget to close the database connection.
// connection.end((err) => {
//   if (err) {
